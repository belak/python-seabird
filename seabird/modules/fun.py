    ...
    def cmd_coin(self, msg):
        ...
        if msg.trailing not in CoinPlugin._coin_names:
            ...
            self.bot.mention_reply(
                msg, "That's not a valid coin side. Options are: {}".format(names)
            )
            return
        ...
        if choice == msg.trailing:
            ...
            self.bot.write(
                "KICK", msg.args[0], msg.prefix.name, "Sorry! Better luck next time!"
            )
    ...
    def irc_privmsg(self, msg):
        ...
        for match in DicePlugin.dice_re.finditer(msg.trailing):
            ...
            if dice_count < 1:
                # Not enough dice
                self.bot.mention_reply(
                    msg, "{} is not a valid number of dice.".format(dice_count)
                )
                return

            if dice_magnitude < 2:
                self.bot.mention_reply(
                    msg, "{} is not a valid die size.".format(dice_magnitude)
                )
                return

            if dice_magnitude > 100:
                self.bot.mention_reply(
                    msg, "Die of size {} is too large".format(dice_magnitude)
                )
                return
